...
def build_frame(comp, prefix, ev):
    ...
    mid = af.off_plane(comp, comp.yZConstructionPlane,
    ...
    leg_l = af.ext_new(comp, pr, "leg_h", f"{p}_LegL").bodies.item(0)
    leg_l.name = f"{p}_Leg_L"
    leg_r = af.mirror_body(comp, leg_l, mid, f"{p}_LegR_Mir").bodies.item(0)
    leg_r.name = f"{p}_Leg_R"

    # Rail (centered on leg in Y)
    rail_pl = af.off_plane(comp, comp.xZConstructionPlane,
                           "(leg_w - rail_t) / 2", f"{p}_Rail_Pl")
    _, pr = af.sketch_rect_model(comp, rail_pl,
        ("leg_w", "(leg_w - rail_t) / 2", "rail_z"),
        {"x": "rail_l", "z": "rail_w"}, f"{p}_Rail_Sk", ev)
    rail = af.ext_new(comp, pr, "rail_t", f"{p}_Rail").bodies.item(0)
    rail.name = f"{p}_Rail"
...
def run(context):
    ...
    left_face = af.find_face(rail, "x", -1)
    right_face = af.find_face(rail, "x", +1)
    ...
    af.combine(bmt_c, leg_l, [rail], CUT, True, "bmt_MortL")
    af.combine(bmt_c, leg_r, [rail], CUT, True, "bmt_MortR")
    ...
    lmt_mid = af.off_plane(lmt_c, lmt_c.yZConstructionPlane,
                            "(2 * leg_w + lmt_span) / 2", "lmt_Mid")

    # Left leg
    _, pr = af.sketch_rect_model(lmt_c, lmt_c.xYConstructionPlane,
        ("0 in", "0 in", "0 in"),
        {"x": "leg_w", "y": "leg_w"}, "lmt_LegL_Sk", ctx.ev)
    leg_l = af.ext_new(lmt_c, pr, "leg_h", "lmt_LegL").bodies.item(0)
    leg_l.name = "lmt_Leg_L"

    # Mirror → right leg
    leg_r = af.mirror_body(lmt_c, leg_l, lmt_mid, "lmt_LegR_Mir").bodies.item(0)
    leg_r.name = "lmt_Leg_R"

    # Top rail: spans full width, sits on top of legs
    rail_pl = af.off_plane(lmt_c, lmt_c.xYConstructionPlane,
                            "leg_h", "lmt_TopRail_Pl")
    _, pr = af.sketch_rect_model(lmt_c, rail_pl,
        ("0 in", "0 in", "leg_h"),
        {"x": "2 * leg_w + lmt_span", "y": "leg_w"}, "lmt_Rail_Sk", ctx.ev)
    top_rail = af.ext_new(lmt_c, pr, "lmt_rh", "lmt_TopRail").bodies.item(0)
    ...
    left_top = af.find_face(leg_l, "z", +1)
    ...
    right_top = af.find_face(leg_r, "z", +1)
    ...
    af.combine(lmt_c, top_rail, [leg_l], CUT, True, "lmt_MortL")
    af.combine(lmt_c, top_rail, [leg_r], CUT, True, "lmt_MortR")
