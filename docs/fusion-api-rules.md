...
| CUT computes "successfully" but removes nothing (body keeps full volume; connectivity/interference/deps all pass) | Profile drawn with raw coordinates on a non-XY plane landed off the body (e.g. xZ plane: sketch-Y = model **-Z**, wedge below the floor); or combine tool doesn't intersect target | `validate_design` now FAILS with the feature named in `featureImpact.zeroImpactFeatures`. Prevent: always `modelToSketchSpace` + `probe_orientations`; verify cuts by VOLUME delta in the script. For leg tapers use the `taper_legs` template (has a built-in raise-on-no-op check). |
| Pattern silently creates a 3×N grid of coincident duplicate bodies | `rectangularPatternFeatures.createInput()` defaults `quantityTwo` to **3** with zero spacing | Always set `directionTwoEntity`, `quantityTwo = VI("1")`, `distanceTwo = VI("0 in")` on the input before `add()`. Verify replication by body COUNT after every pattern. |
| Feature-pattern of a Combine fails `IDENTICAL_TARGET_AND_TOOL_BODY` | The replayed Combine doesn't remap target/tool to the instance copies; multi-feature input sets also duplicate the whole dependency chain once PER input feature | Bodies meant for pattern/mirror replication must be a SINGLE extrude (e.g. an L-profile in one sketch instead of two plates + JOIN). |
| Mirror/pattern copies get wrong names; source bodies renamed | `MirrorFeature.bodies` can include the INPUT bodies, not just the new copies | Rename via a before/after body-name snapshot of the component, never by indexing `.bodies`. |
| Mirror reports 0 bodies and 0 faces but worked fine | A later JOIN consumed the mirror's products (e.g. mirrored tenons joined into their rail) — ownership moved downstream | Not a bug — this is why `featureImpact` does NOT flag mirrors/patterns. Verify replication by body count in the build script instead. |
