...
- **Re-binding (rare).** If a session loses its document binding — e.g. after an add-in restart — the next call returns a "session restored" message listing available documents with their `doc_key`s. Only then call `claim_document` (preferably by `doc_key`) to re-adopt your document. In normal operation you never call it.
