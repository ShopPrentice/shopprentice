...
| `grain_vector` | `(body)` | Vector3D | Fiber/grain direction of a body as a unit vector, from the principal axes of inertia (smallest moment = elongation = fiber); falls back to the longest bbox axis. Works for slender and angled members. |
| `grain_axis` | `(body)` | str | Fiber/grain direction as an axis name `'x'`/`'y'`/`'z'` (longest bbox axis). |
| `tenon_wide_axis` | `(mortise_body, tenon_axis)` | str | The perpendicular model axis the tenon's WIDER cross-section dimension should run, so the mortise is cut WITH the mortise piece's grain. Build the narrow dimension along the other perpendicular axis. See `docs/joinery/grain-and-strength.md`. |
| `tenon_wide_direction` | `(mortise_body, tenon_axis)` | Vector3D or None | Vector form of `tenon_wide_axis` (the mortise fiber projected ⊥ the insertion axis); `None` for an end-grain mortise (fiber ∥ insertion). |
| `validate_tenon_grain` | `(tenon_body, mortise_body, tenon_axis, tol=0.12)` | dict | Verifies a tenon's wider cross-section dimension runs ALONG the mortise grain. Prints a **WARNING** and returns `{ok:False,...}` if the tenon is wider ACROSS the grain (90°-wrong, severs extra fibers). Square sections pass. Never raises — call on the tenon body before JOIN. |
| `validate_mortise_tenon` | `(tenon_body, mortise_body, tenon_axis, species="hardwood", through=, proud=, sized=, expected=, thin_ratio=0.25)` | dict | Build-time M&T strength GATE: measures the tenon off the body, runs the grain check + `joint_strength.mortise_tenon_flags`, prints a **WARNING** on load-independent red flags (grain-wrong, thin slice t<¼w, very-thin) + optional `expected={mode:lbf}` overload. Returns `{ok, flags, weakest, dims_in, capacities, utilization, estimate}`. Never raises. The `mortise_tenon` template auto-calls it; call it yourself for hand-built joints before JOIN. |
| `validate_pegged_joint` | `(tenon_body, mortise_body, tenon_axis, pins, pin_dia, pin_end_distance, species="hardwood", peg_species=None)` | dict | **Dedicated** build-time check for a drawbore/pegged tenon (split from the M&T check): relish tear-out (peg end distance ≥ 4×D) + the European-Yield-Model peg capacity. Returns `{ok, flags, pin_modes, pin_withdrawal, dims_in}`. Never raises. |
| `validate_joint_strength` | `(tenon_body, mortise_body, tenon_axis, species, through=, proud=, pins=, pin_dia=, pin_end_distance=, sized=, expected=)` | dict | **DEPRECATED** back-compat shim — use `validate_mortise_tenon` (+ `validate_pegged_joint` for pegs). Runs the M&T check and, when `pins` given, the pegged check, merging flags. |
| `validate_joints` | `(ctx, joints)` | dict | Runs the per-type check on each declared joint from `model.json`'s `joints` array (dispatch via `validate_joint`). Declaration errors (unknown type / missing field / unresolved body) **fail**; strength findings are advisory WARNINGs. Returns `{ok, n_warn, n_err, results}`. Called automatically by `validate_deps` when a `joints` array is present. |
| `validate_joint` | `(decl, ctx)` | dict | Validate one declared joint, dispatching on `decl["type"]` (`mortise_tenon`, `pegged_tenon`/`drawbore`, `wedged_tenon`). Resolves the owning bodies, evaluates the declared dims (expression strings → `ctx.ev`), runs the pure flag cores + mortise-grain sanity. Never raises. Returns `{type, tenon, mortise, ok, flags, errors, notes}`. |
| `declare_joint` | `(decl, metadata_path=None)` | bool | Auto-declare: append a joint decl to `model.json`'s `joints` array, deduped by `(type, sorted body-pair)` (updates in place on rebuilds), preserving `deps`. No-ops (returns False) when `model.json` isn't resolvable/absent (tests, sandbox). Joinery templates call this. |
| `contacting_pairs` | `(ctx, min_area_cm2=1.0)` | list | Body-name pairs whose planar faces are in face-to-face contact, design-wide (planar-overlap test mirroring `validate_design`). Fail-soft — returns `[]` offline. Used by the deps completeness pass to surface contacting bodies with no declared joint. |
| `joint_strength.mortise_tenon_flags` / `pegged_flags` | `(width, thickness, depth, …)` / `(pins, pin_dia, pin_end_distance, …)` | dict | **Pure** per-type flag cores (`from helpers.sp.joint_strength import …`; no Fusion). The shared logic behind both the body-mode gate and the registry check — offline-testable. |
| `joint_strength.estimate_mortise_tenon` | `(width, thickness, depth, species=, through=, sized=, pins=, pin_dia=, pin_end_distance=, ...)` | dict | **Design-time** strength estimator (`from helpers.sp.joint_strength import ...`; pure Python, ~5µs). Capacity in every direction (withdrawal/thrust/shear_along_w·t/bending/torsion/pin) + governing mode + guidance. `summarize(result)` prints it; `glue_shear_per_area(angle)` for off-axis faces (Hankinson). See `docs/joinery/grain-and-strength.md`. |
...
| `reanchor` | `(sk, parent_body, parent_occ, face_axis, face_dir, anchor_xyz)` | Profile (re-resolved) or None | **One-call fix for a non-root sketch built in origin mode.** Projects the parent face, finds a non-origin anchor, and retargets every `sk.originPoint` dimension to it as `abs(<orig expr> - <anchor axis expr>)`. DOF + geometry preserved. **Returns a FRESH profile — extrude that, not one captured before the call (the projection invalidates the earlier profile).** `anchor_xyz` = `(x,y,z)` model exprs of a real parent corner. The anchored vertex must not itself be at world (0,0) — for that use `sketch_rect_model(anchor=…, size_far=True)`. |
| `validate_deps` | `(ctx, metadata_path=None)` | bool or None | Validates the dependency tree from `model.json`. Hard checks (affect pass/fail): (1) single origin root — only 1 body may reference `"origin"`; (2) sketch origin — non-root sketches must not dimension from `sk.originPoint`; (3) **sketch traceability** — every non-root sketch must project real reference geometry, use no Fix/Ground constraint on drawn geometry, and be fully constrained relative to that reference (Fusion's `sketch.isFullyConstrained` is the judge — a rigid sketch that floats is not fully constrained; only fit-point spline interiors may remain free, and their start/end must anchor); (4) no bodies in the root component. Advisory: completeness (tracked-body coverage). Returns `True`/`False`/`None` (no metadata). Dep entries support optional `"replicas": "glob_pattern"` for pattern copies. A body's `"ref"` may be a single parent name OR a list of names — multi-contact joints reference more than one parent (e.g. a wedge bearing on a post AND riding a stretcher; a shelf tenoned into two sides). Only the `"origin"` root is unique; every other body needs ≥1 parent but is not capped at one. Optional top-level **`"joints"`** array (issue 106): each entry `{type, tenon, mortise, axis, …}` runs a per-type strength check (`validate_joints`) and the completeness pass flags contacting bodies with no declared joint. See `docs/joinery/grain-and-strength.md`. |
