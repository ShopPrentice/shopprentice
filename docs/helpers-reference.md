...
| `validate_joint_strength` | `(tenon_body, mortise_body, tenon_axis, species="hardwood", through=, proud=, pins=, pin_dia=, pin_end_distance=, sized=, expected=)` | dict | Build-time strength GATE: measures the tenon off the body, runs `estimate_mortise_tenon`, and prints a **WARNING** on load-independent red flags (grain-wrong, thin slice t<¼w, brittle peg <4×D) + optional `expected={mode:lbf}` overload. Returns `{ok, flags, weakest, dims_in, capacities, utilization}`. Never raises. The `mortise_tenon` template auto-calls it; call it yourself for hand-built joints before JOIN. |
...
| `joint_strength.mortise_tenon_flags` / `pegged_flags` | `(width, thickness, depth, …)` / `(pins, pin_dia, pin_end_distance, …)` | dict | **Pure** per-type flag cores (`from helpers.sp.joint_strength import …`; no Fusion). The per-joint-type logic behind the registry check (`validate_joint`) — offline-testable. |
