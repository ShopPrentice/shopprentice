...
def single(comp, plane, center, long_axis, long_expr, short_expr,
    ...
    if cut:
        # combine_auto places the CUT intra-component when body_a /
        # body_b share a component with the void, or at root with
        # assembly proxies when they don't.
        sp.combine_auto(body_a, void_body, CUT, True, f"{name}_CutA")
        sp.combine_auto(body_b, void_body, CUT, True, f"{name}_CutB")
...
def grid(comp, plane, start, step_axis, step_expr, count_expr,
    ...
    # Bulk CUT all voids into target bodies (combine_auto handles
    # cross-component routing when body_a / body_b live elsewhere).
    if cut and void_bodies:
        sp.combine_auto(body_a, void_bodies, CUT, True, f"{name}_CutA")
        if body_b is not None and body_b != body_a:
            sp.combine_auto(body_b, void_bodies, CUT, True, f"{name}_CutB")
...
def between(comp, plane, body_a, body_b, interface_axis,
    ...
    # Bulk CUT into both bodies (combine_auto routes cross-component).
    if cut and void_bodies:
        sp.combine_auto(body_a, void_bodies, CUT, True, f"{name}_CutA")
        if body_b is not None and body_b != body_a:
            sp.combine_auto(body_b, void_bodies, CUT, True, f"{name}_CutB")
...
def four_corners(comp, plane, center, long_axis, long_expr, short_expr,
    ...
    # 4. CUT all into top/seat — combine_auto routes cross-comp when
    # top_body lives in a different component than the voids.
    sp.combine_auto(top_body, all_voids, CUT, True, f"{name}_Top_Cut")

    # 5. CUT each into its leg (keepTool=True on all). Each leg may
    # live in its own component.
    leg_nl, leg_nr, leg_fl, leg_fr = leg_bodies
    sp.combine_auto(leg_nl, [nl_body], CUT, True, f"{name}_Leg_NL")
    sp.combine_auto(leg_nr, [nr_body], CUT, True, f"{name}_Leg_NR")
    sp.combine_auto(leg_fl, [fl_body], CUT, True, f"{name}_Leg_FL")
    sp.combine_auto(leg_fr, [fr_body], CUT, True, f"{name}_Leg_FR")
