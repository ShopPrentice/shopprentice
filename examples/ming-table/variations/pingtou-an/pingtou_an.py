"""Large 平头案 (flat-end recessed-leg table) — 60 x 25 x 30 in.
The Ming side table scaled up; same joinery, no shelf, plus two round
stretchers (coped saddle shoulders + blind four-shoulder tenons).

Component-organized parametric build:
  Legs / Top (flush frame-and-panel + dovetail battens) / Aprons / Stretchers
...
def run(context):
    ...
    # --- Envelope ---
    add("table_l", "60 in", "in", "Overall table length (X)")
    add("table_d", "25 in", "in", "Overall table depth (Y)")
    add("table_h", "30 in", "in", "Overall table height, floor to top surface")
    add("splay", "1.5 deg", "deg", "Leg splay/rake angle from vertical, per axis")
    # --- Top frame & panel ---
    add("tf_t", "1.25 in", "in", "Top-frame stock thickness (vertical)")
    add("tf_w", "4 in", "in", "Top-frame member width")
    ...
    add("leg_dia", "2 in", "in", "Leg diameter (round leg)")
    add("leg_r", "leg_dia / 2", "in", "Leg radius (derived)")
    add("leg_setback_x", "8 in", "in", "Distance each leg is set in from the table END (X)")
    # apron stock + end inset defined here (before leg_setback_y) so the side setback
    ...
    # --- Apron --- (apron_t defined above with the leg setbacks)
    add("apron_w", "2.4 in", "in", "Apron height (band width)")
    ...
    add("fbd_tail_w", "0.35 in", "in", "Dovetail tail width at wide face")
    ...
    add("spandrel_depth", "5.4 in", "in", "Spandrel vertical extent below the apron")
    ...
    add("bt_off", "(table_l / 2 - tf_w) / 2", "in", "Top-batten offset from center (outer pair; quarter points)")
    ...
    add("slot_w", "2.5 in", "in", "Leg slot width (spans the leg in X)")
    ...
    def batten(name, cxb):
        ...
        ys = (ev("table_d") / 2 - ev("tf_w")) / IN   # frame inner face (in) = table_d/2 - tf_w
        def CX(e):
    ...
    bt_r = batten("Batten_R", ev("bt_off") / IN)
    ...
    batten("Batten_C", 0)   # 3 battens total: outer pair at +/-bt_off + a centre one
    # battens: sliding-dovetail socket in panel + tenon socket in frame
    for bn in ("Batten_R", "Batten_L", "Batten_C"):
    ...
    for fn in ("TF_Front", "TF_Back"):
        sp.combine(ctx.find_body(fn),
                   [ctx.find_body("Batten_R"), ctx.find_body("Batten_L"),
                    ctx.find_body("Batten_C")], CUT, True, fn + "_BtMort")
    ...
    add("cove_r", "1 in", "in", "Apron/spandrel cove radius")
    add("bot_r", "1 in", "in", "Spandrel lower-corner transition radius")
    # (ap_end_inset defined up top with the leg setbacks)
    add("la_half", "table_l / 2 - ap_end_inset", "in", "Apron half length (derived)")
    add("sp_edge_gap", "1.25 in", "in", "Spandrel edge to leg surface")
    ...
    def long_side(name):
        ...
        # Use only TRULY sketch-horizontal frame edges (parallel to the band bottom);
        # the 格角榫 miters / leg-cuts project as ~splay-deg lines that pass the |dx|>|dy|
        # filter but aren't parallel, so addOffsetDimension can't solve against them.
        # Among the horizontal ones, the edge whose world-Z is nearest tf_bot is the
        # real underside (robust as the side-setback / projection drift grows).
        def _wz(l):
            return (l.startSketchPoint.worldGeometry.z
                    + l.endSketchPoint.worldGeometry.z) / 2.0
        _hh = [l for l in horiz if hv(l)[3] < 0.02 * hv(l)[2]]
        underside = min(_hh or horiz, key=lambda l: abs(_wz(l) - ev("tf_bot")))
        ...
        # Fixed poke-through above the TRUE frame underside (band positioned by its
        # BOTTOM at tf_bot-apron_w plus a fixed height) — robust to the projection drift
        # that goes strongly negative as the leg side-setback grows.
        _overlap = ev("apron_t") * math.tan(ev("splay")) + 0.08 * IN   # cm (thin trim sliver)
        ...
        d = skc.sketchDimensions
        def offd(a, b, expr):
            # text point at the centroid of the two entities — on a large part the sketch
            # origin is far off and makes addOffsetDimension resolve the offset side wrong.
            ps = [a.startSketchPoint.geometry, a.endSketchPoint.geometry,
                  b.startSketchPoint.geometry, b.endSketchPoint.geometry]
            tp = P(sum(p.x for p in ps) / 4.0, sum(p.y for p in ps) / 4.0, 0)
            d.addOffsetDimension(a, b, tp).parameter.expression = expr
        offd(underside, lns[2], "apron_w - (%.6f cm)" % _band_corr)  # band BOTTOM at true tf_bot-apron_w
        # band HEIGHT (left-end length) = apron_w + overlap -> top a fixed amount above the
        # frame, NOT offset from the drifting projected underside.
        _VER = adsk.fusion.DimensionOrientations.VerticalDimensionOrientation
        _g11 = lns[11].startSketchPoint.geometry
        d.addDistanceDimension(lns[11].startSketchPoint, lns[11].endSketchPoint, _VER,
            P(_g11.x - 0.5, _g11.y, 0)).parameter.expression = "apron_w + (%.6f cm)" % _overlap
        ...
        for (pr, _ap), lr in zip(spp, ("L", "R")):
            ...
            sb.name = spref + lr
        # ---- Connecting pieces: apron_t/4 tongue at apron center, bridging band↔spandrel ----
        ...
        # The split line must sit UNDER the apron band's lowest spot (AB = tf_bot -
        # apron_w) so the tongue bridges the band down INTO the spandrel. The old
        # "1 7/16 below the section top" landed ABOVE the band bottom once the apron
        # grew taller. Solve the model-z -> sketch-y mapping with two probe points
        # (robust to the tilted plane and either sketch-y orientation).
        s2m = csk.sketchToModelSpace
        mid_x = (min_x + max_x) / 2.0
        pA = s2m(P(mid_x, min_y, 0)); pB = s2m(P(mid_x, min_y + 1.0, 0))
        dz = pA.z - pB.z                                   # model-z drop per +1 sketch-y
        target_z = ev("tf_bot") - ev("apron_w") - 0.5 * IN  # 1/2" below the band bottom
        cut_y = min_y + (pA.z - target_z) / dz
        ...
        # Select the profile ON TOP of the split line: sketch -y is model up, so the
        # upper portion lies entirely at y <= cut_y. Use the profile's geometric
        # boundingBox — profileCurves.sketchEntity endpoints are the FULL entity's
        # ends (the cut splits profiles, not entities), so vertex scans see points
        # beyond the profile and can match/reject the wrong fragment. That's how the
        # big BELOW-line profile got selected before (last gable-peak match won).
        conn_pr = None; conn_area = 0.0
        for pi in range(n_pr):
            pr = csk.profiles.item(pi)
            bb = pr.boundingBox
            a = pr.areaProperties().area
            print("  profile %d: area=%.2f bb_y=[%.2f, %.2f] cut_y=%.2f" %
                  (pi, a, bb.minPoint.y, bb.maxPoint.y, cut_y))
            if bb.maxPoint.y <= cut_y + 0.05 and a > conn_area:
                conn_pr = pr; conn_area = a
                print("  -> SELECTED (entirely above split line, area=%.2f)" % a)
        # tongue thickness = apron_t/4 (sym half = apron_t/8) — scales with the stock
        conn_R = sp.ext_new_sym(apc, conn_pr, "apron_t / 8", name + "_ConnR").bodies.item(0)
    ...
    # ============================================================
    # STRETCHERS (component): 2 ROUND stretchers, one per END, running
    # front-to-back at the lower 1/3 of leg height. Section = 2/3 leg dia;
    # coped (saddle) shoulder wraps each round leg (the leg cuts the
    # stretcher, like the shelf rails); blind tenon into the leg (stopped
    # mortise), recessed on all four sides -> shoulders top/bottom too.
    # ============================================================
    occ_str = sp.make_comp(root, "Stretchers")
    strc = occ_str.component
    add("stretcher_dia", "leg_dia * 0.8", "in", "Round stretcher diameter (0.8 of leg)")
    add("stretcher_r", "stretcher_dia / 2", "in", "Stretcher radius (derived)")
    add("stretcher_z", "table_h / 3", "in", "Stretcher centerline height (lower 1/3)")
    add("str_tenon_d", "leg_r * 1.5", "in", "Blind tenon depth into the leg (stopped, not through)")
    add("str_tenon_w", "stretcher_dia / 3", "in", "Tenon width (side shoulders)")
    add("str_tenon_h", "stretcher_dia * 2 / 3", "in", "Tenon height (top/bottom shoulders)")
    add("leg_x_str", "ltx + (leg_tip_z - stretcher_z) * tan(splay)", "in", "Leg X at stretcher height (derived)")
    add("leg_y_str", "lty + (leg_tip_z - stretcher_z) * tan(splay)", "in", "Leg Y at stretcher height (derived)")

    def build_stretcher(sx, lbl, legF, legB):
        Xc = sx * ev("leg_x_str"); Yh = ev("leg_y_str")
        z = ev("stretcher_z"); r = ev("stretcher_r"); lr = ev("leg_r"); tw = ev("str_tenon_w")
        # round body, axis Y, leg-centre to leg-centre (extends into both legs)
        pl = sp.off_plane(strc, strc.xZConstructionPlane, "%.6f cm" % -Yh, lbl + "_Pl")
        sk = strc.sketches.add(pl); sk.name = lbl + "_Sk"
        m = sk.modelToSketchSpace; c = m(P(Xc, -Yh, z))
        cir = sk.sketchCurves.sketchCircles.addByCenterRadius(P(c.x, c.y, 0), r)
        sk.sketchDimensions.addRadialDimension(
            cir, P(c.x + r, c.y, 0)).parameter.expression = "stretcher_r"
        body = sp.ext_new(strc, sk.profiles.item(0), "2 * leg_y_str", lbl).bodies.item(0)
        body.name = lbl
        # cope: the two end legs cut the round body -> saddle shoulders wrapping each leg
        sp.combine(proxy(lbl, occ_str),
                   [proxy(legF, occ_legs), proxy(legB, occ_legs)], CUT, True, lbl + "_Cope")
        # blind tenons into each leg (tw wide, str_tenon_h tall — centered on the
        # stretcher axis so it has shoulders on ALL four sides; str_tenon_d deep,
        # overlapping the coped stretcher end by 0.3 cm so the JOIN merges them)
        yF = -Yh + lr; yB = Yh - lr
        boxcm(strc, lbl + "_TnF", Xc - tw / 2, Xc + tw / 2,
              yF - ev("str_tenon_d"), yF + 0.3, "stretcher_z - str_tenon_h / 2", "str_tenon_h")
        boxcm(strc, lbl + "_TnB", Xc - tw / 2, Xc + tw / 2,
              yB - 0.3, yB + ev("str_tenon_d"), "stretcher_z - str_tenon_h / 2", "str_tenon_h")
        sp.combine(proxy(legF, occ_legs), [proxy(lbl + "_TnF", occ_str)], CUT, True, lbl + "_MortF")
        sp.combine(proxy(legB, occ_legs), [proxy(lbl + "_TnB", occ_str)], CUT, True, lbl + "_MortB")
        sp.combine(ctx.find_body(lbl),
                   [ctx.find_body(lbl + "_TnF"), ctx.find_body(lbl + "_TnB")], JOIN, False, lbl + "_TnJ")
    build_stretcher(-1, "Stretcher_L", "Leg_FL", "Leg_BL")
    build_stretcher(1, "Stretcher_R", "Leg_FR", "Leg_BR")
    ...
    allb = []
    for occ in (occ_legs, occ_top, occ_ap, occ_str):
        c = occ.component
