...
1. **Plan before building.** Before writing code, outline every modeling step in order: which component, which feature, which replication strategy. Think: "If I were clicking through the Fusion 360 UI, what would I do next?" Write the plan as a step list (see Design-First Planning below).
   ...
   **Show the concept first (Claude Code, optional).** If you have `show_widget` (visualize MCP) *and* can spawn sub-agents, sketch a non-obvious joint to scale for the user before committing geometry — see `docs/visual-communication.md`. Lacking either tool (e.g. Codex): skip this note entirely — use text + post-build screenshots. It changes nothing about the build.

2. **Build one, replicate the rest.** Prefer building one template and using **Mirror** and **Rectangular Pattern** features for the rest. If you find yourself reaching for a Python `for` loop to create geometry, stop — use a Fusion 360 pattern instead. Building each congruent part from its own sketch is also the *token-expensive* path; one template + a pattern is cheaper AND parametric. **Exception:** Per-corner joinery (dovetails, box joints) where CUT/JOIN targets differ per corner requires independent construction at each corner — mirrors of CUT/JOIN extrudes inherit the original `participantBodies` reference and fail. **Design replicable parts as a SINGLE extrude** (Combine-built parts can't be feature-patterned) and verify every pattern/mirror by BODY COUNT — the silent failure modes are in the `docs/fusion-api-rules.md` error table.
...
| **Details & Finishing** | Fillets, chamfers, edge treatments, leg tapers — use the `taper_legs` template, never hand-rolled wedge sketches (Phase 3) | Tested (white-oak table; straight/rake/splay fixtures) | `docs/details-and-finishing.md` |
...
| **Visual Communication** | **Optional, Claude Code only** (needs `show_widget`/visualize MCP + sub-agents; Codex etc. skip). When you have both: sketch a joint / cross-section / structural choice for the user with `show_widget` *before* building to confirm intent — a sub-agent authors the SVG, you render it | Draft | `docs/visual-communication.md` |
...
5. Ask: "Do two parts meet or pass each other inside a third?" Fit dimensions that can collide (e.g. tenon depths meeting inside a leg) must be DERIVED with explicit clearance, never fixed at a value that exactly touches: `tenon_depth = "leg_size - (apron_thick + tenon_thick) / 2 - 0.0625 in"`, not `"1 in"`.
...
3. **`validate_parametrics` → passed** — ±5% sweep of every leaf parameter; run it on a freshly rebuilt or saved document. Fix fragile parameters by deriving the fit (Parameter Planning #5). Caveats + resume path: `docs/incremental-updates.md` § Parameter Robustness.
