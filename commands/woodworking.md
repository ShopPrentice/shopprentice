...
2. **Build one, replicate the rest.** Prefer building one template and using **Mirror** and **Rectangular Pattern** features for the rest. If you find yourself reaching for a Python `for` loop to create geometry, stop — use a Fusion 360 pattern instead. Building each congruent part from its own sketch is also the *token-expensive* path; one template + a pattern is cheaper AND parametric. **Exception:** Per-corner joinery (dovetails, box joints) where CUT/JOIN targets differ per corner requires independent construction at each corner — mirrors of CUT/JOIN extrudes inherit the original `participantBodies` reference and fail. **Design replicable parts as a SINGLE extrude:** a part built as plates + Combine cannot be feature-patterned (the replayed Combine fails with IDENTICAL_TARGET_AND_TOOL_BODY), and multi-feature pattern inputs duplicate the whole chain once per input. After every pattern/mirror, verify by BODY COUNT — `RectangularPatternFeatures.createInput` defaults `quantityTwo` to 3 with zero spacing (silent stacked duplicates), and a mirror whose products a later JOIN consumed legitimately reports 0 bodies/faces.
...
| **Details & Finishing** | Fillets, chamfers, edge treatments, leg tapers (Phase 3) — includes the `taper_legs` template (orientation-agnostic taper cuts anchored to the leg's own edges; works on straight, raked, and splayed legs; raises on no-op cuts). NEVER hand-roll taper wedge sketches with raw plane coordinates: on the xZ plane sketch-Y = model -Z and the cut silently removes nothing | Tested (white-oak table; straight/rake/splay fixtures) | `docs/details-and-finishing.md` |
...
5. Ask: "Do two parts meet or pass each other inside a third?" Fit dimensions that can collide (tenon depths meeting inside a leg, parts passing in a shared cavity) must be DERIVED with explicit clearance — never fixed at a value that exactly touches. A fixed `tenon_depth = "1 in"` that corner-touches at the defaults collides the moment `leg_size` shrinks or `apron_thick` grows; `tenon_depth = "leg_size - (apron_thick + tenon_thick) / 2 - 0.0625 in"` keeps 1/16" clearance at any size.
...
3. **`validate_parametrics` → passed** (run on a freshly rebuilt or saved document — the sweep mutates and reverts the doc, and on complex face-referencing joinery a layout change can kill cached references irreversibly). It perturbs every leaf parameter ±5% one at a time and names fragile parameters with the exact symptom (missing bodies, stranded lumps, new warnings, zero-impact cuts, interference, floating parts). If it aborts, rebuild from the script and re-run with `params=untestedParameters`. Fix fragile parameters by deriving the fit dimension (see Parameter Planning #5), not by avoiding the parameter.
...
8. **`validate_design` → passed.** Single call checks connectivity (1 cluster) + interference (0 real overlaps) + **feature impact** (every CUT/INTERSECT must actually change geometry — a cut whose profile misses its target computes "successfully" but removes nothing; zero-impact features FAIL and are named) + dependency tree (single origin root, sketch origin enforcement, bodies in components). It also prints a **replication advisory** (congruent bodies built independently → Mirror/Pattern) — advisory only, does NOT affect pass/fail; address it per rule 2 (sub-agent refactor) or tag with `_norep`. Run after EVERY phase.
