...
**Multi-agent / parallel sessions.** When several agents work concurrently (e.g. a fan-out where each agent builds or fixes a different piece), the **Session Manager assigns and maintains a dedicated Fusion document per agent**, keyed by your session ID. Your `execute_script` / `capture_design` / `validate_design` calls automatically operate on YOUR document — you never choose, create, or `claim_document` one, and `clean=True` rebuilds only your own document, so parallel agents never collide. Fusion runs commands one at a time; the Session Manager queues and spaces requests across all agents so Fusion stays responsive — expect your calls to serialize with other agents' (a little extra latency) with no effect on correctness. So run the normal execute → validate loop exactly as a solo agent would; you do NOT need to coordinate documents or take turns. (Only if a session loses its binding — e.g. after an add-in restart — do you get a "session restored" message listing `doc_key`s; then call `claim_document` to re-bind. Otherwise you never need it.)
