...
| **Incremental Updates & Build Strategy** | Build order, component-by-component workflow, document management, script epilogue, interactive editing, rebuild-vs-patch. **Size variations / resizing an existing model** (propose the full proportion set up front, audit baked constants, datum-relative selection heuristics, volume-scaling validation) — read BEFORE building "the same piece at a different size" | Tested (Ming table → 平头案) | `docs/incremental-updates.md` |
